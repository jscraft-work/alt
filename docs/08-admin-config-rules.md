...
| 연결 계좌 | 가능 | 불가 | 불가 | 가능 |
| 전략 예산 | 가능 | 불가 | 불가 | 불가 |
| 프롬프트(신규 버전, frontmatter+본문) | 가능 | 가능(다음 사이클부터) | 가능 | 가능 |
| 감시 종목 | 가능 | 가능 | 가능 | 가능 |
| 트레이딩 모델 | 가능 | 가능 | 가능 | 가능 |
...
3. 활성 prompt 본문 상단의 YAML frontmatter(prompt 메타데이터 블록)가 `PromptInputSpecParser`로 정상 파싱될 것 (구 "유효한 전략 입력 스펙" 항목 대체)
4. 유효한 인스턴스 실행 주기가 결정되어 있을 것
5. 전략 예산이 설정되어 있을 것
6. 실행 모드가 `live`라면 연결 계좌가 선택되어 있을 것
7. 평가 대상 범위(frontmatter `scope`)가 `full_watchlist`라면 감시 종목이 1개 이상일 것
...
- 입력 스펙은 더 이상 별도 JSON 컬럼이 아니므로, 활성 prompt 본문 frontmatter 파싱 가능 여부로만 판정한다.

응답 규칙:

- 검증 실패는 `INSTANCE_NOT_ACTIVATABLE` 에러로 응답한다.
- `meta.missing[]`에 미충족 항목 코드를 모두 담는다 (`prompt`, `model`, `promptFrontmatter`, `cycleMinutes`, `budget`, `account`, `watchlist`).
- 부분 활성화는 없다. 한 항목이라도 미충족이면 `active` 전이를 거절한다.
- `promptFrontmatter` 미충족의 경우 `message`에 파싱 실패 사유를 함께 담는다.
...
- 복제 대상은 **설정만**이다.
- 복제되는 값: 프롬프트(frontmatter+본문), 감시 종목, 모델, 시스템 파라미터, 예산, 모드, 연결 계좌
...
- 스냅샷 대상: 프롬프트(현재 버전, frontmatter + 본문 템플릿 일체), 모델, 실행 주기, 예산, 감시 종목, 인스턴스 단위 시스템 파라미터.
- 입력 스펙은 더 이상 별도 스냅샷 항목이 아니다 — prompt 본문 안 frontmatter가 그 역할을 겸한다.
